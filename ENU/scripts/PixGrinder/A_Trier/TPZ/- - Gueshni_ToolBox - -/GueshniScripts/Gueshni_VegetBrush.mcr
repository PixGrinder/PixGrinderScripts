macroScript VegetBrush category:"- Gueshni -" Icon:#("g9_vegetbrush", 1) tooltip:"Veget Brush"
 (
	 on execute do
	(
		rollout params "Paramètres" width:162 height:300
(
	fn getpos ir =
	(
		local found = false
	...
	(
		...
		if selection.count == 0 then
			(
				messagebox "Sélection vide" ; delete objfinal ; params.ckb1.state = off; #stop
			)
			else
			(
			 	if shiftkey then
	...
	label lbl4 "Sélectionner plusieurs objets, cliquer sur \" Dupliquer \" puis brosser en maintenant le bouton gauche enfoncé." pos:[8,6] width:150 height:53
	groupBox grp3 "Contrôles clavier" pos:[4,370] width:150 height:110
	label lbl5 "Shift + Glisser pour changer manuellement d'objet. \n Alt + Glisser pour repasser en mode aléatoire." pos:[8,390] width:130 height:90
...
(
label lbl6 "1- L'altimétrisation ne fonctionne pas en vue Utilisateur. Travailler en vue Perspective, Caméra ou Dessus.\n\n2- Si l'orientation est déterminée suivant le sens du glissé, le premier objet créé est mal orienté." pos:[8,6] width:150 height:200
)
