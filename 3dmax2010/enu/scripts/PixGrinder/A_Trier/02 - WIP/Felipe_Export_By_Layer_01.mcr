...
macroScript Export_By_Layer
category:"Felipe Scripts"
toolTip:"Export Each Scene Layer Separately"
...
--------------------
	...
	(
		...
		for j = 1 to layermanager.count-1 do (
			...
			if findstring lx.name "zz" == undefined do
			(
			d = #()
			lx.on = true
			lx.select on
...
(
	...
	for j = 1 to layermanager.count-1 do (
		lx = layermanager.getLayer j
		if findstring lx.name "zz" == undefined do append ary lx.name
	)
...
-------------------
x = layermanager.getLayer 2
x.name
if findstring x.name "zz" == undefined do 